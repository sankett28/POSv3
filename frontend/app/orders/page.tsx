...
export default function OrdersPage() {
  ...
  const totalCGST = useMemo(() => Math.round((totalTax / 2) * 100) / 100, [totalTax])
  const totalSGST = useMemo(() => totalTax - totalCGST, [totalTax, totalCGST])  // Ensure exact balance
  const grandTotal = useMemo(() => 
    billItems.reduce((sum, item) => sum + (item.preview_total || item.subtotal), 0),
    [billItems]
  )
      ...
      <div className="max-w-7xl mx-auto w-full">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-4 sm:gap-6 w-full">
          ...
          <div className="xl:col-span-4 lg:col-span-5 bg-white rounded-2xl shadow-md p-4 sm:p-6 border border-[#E5E7EB] xl:sticky xl:top-4 xl:self-start">
            ...
            <div className="space-y-2 mb-3 sm:mb-4 pt-3 sm:pt-4 border-t border-[#E5E7EB]">
              <div className="flex justify-between text-xs sm:text-sm text-[#6B6B6B]">
                <span>Subtotal</span>
                ...
                <>
                  {totalCGST > 0 && totalSGST > 0 ? (
                    <>
                      <div className="flex justify-between text-xs sm:text-sm text-[#6B6B6B]">
                        <span>CGST</span>
                        <span className="whitespace-nowrap">₹{totalCGST.toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between text-xs sm:text-sm text-[#6B6B6B]">
                        <span>SGST</span>
                        <span className="whitespace-nowrap">₹{totalSGST.toFixed(2)}</span>
                      </div>
                    </>
                  ) : (
                    <div className="flex justify-between text-xs sm:text-sm text-[#6B6B6B]">
                      <span>Tax (GST)</span>
                      <span className="whitespace-nowrap">₹{totalTax.toFixed(2)}</span>
                    </div>
                  )}
                </>
              )}
